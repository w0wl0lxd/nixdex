...
| 4 | `nixdex-daemon` `/health` endpoint | Container/ systemd health checks need a cheap probe. | `daemon.rs` only exposes `/locate`. |
...
| 6 | `nixdex-daemon` `/nix-locate` endpoint | `/locate` only maps basename → package list. A full `/nix-locate` query supports `regex`, `package`, `hash`, `type`, `at_root`, `whole_name`, `limit`, `json`. | `database.rs` `SearchOptions` already captures these flags; daemon just needs a handler. |
...
#6 is the natural follow-up that reuses #1. #7–#10 are larger or lower-leverage and
should be deferred to P6 unless a specific use-case arises.
