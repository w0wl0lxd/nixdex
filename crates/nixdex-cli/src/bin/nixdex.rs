...
fn run_generate_man(opts: GenerateManOpts) -> color_eyre::Result<()> {
    ...
    let pages = [
        ...
        (String::from("nix-index"), index::Args::command().name("nix-index")),
        (String::from("nix-locate"), locate::Opts::command().name("nix-locate")),
    ];
    ...
    for (name, cmd) in pages {
        ...
        buf.flush()
            .wrap_err_with(|| format!("failed to flush man page for {name}"))?;
    }
