...
use crate::tui::app::{DetailView, SearchMode};
...
fn perform_package_search(app: &mut App, query: &str) {
    ...
    match matches {
        Ok(records) => {
            let results: Vec<app::SearchResult> = records
                .into_iter()
...
fn perform_locate_search(app: &mut App, query: &str) {
    ...
    match nixdex_core::database::search_results(&options, None) {
        Ok(results) => {
            let search_results: Vec<app::SearchResult> = results
                .into_iter()
    ...
    match nixdex_core::command_index::CommandIndex::open(&app.database) {
        Ok(index) => {
            let providers = index
                ...
                .unwrap_or_default();
            let results: Vec<app::SearchResult> = providers
                .into_iter()
                .map(|p| app::SearchResult {
                    attr: p.attr.clone(),
                    name: p.output.clone(),
                    description: String::new(),
                    path: None,
